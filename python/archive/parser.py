...
def if_exist_news(url,theme):
      sql="SELECT id FROM " + table_news + " WHERE url='" + url + "' AND " + "theme='"+theme+"'"
      cur=sql_query(sql)
      if cur.rowcount < 1:
          return False
      else:
...
def if_exist_currency(news_id,currency,percent,date):
      sql="SELECT id FROM " + table_buy + " WHERE news_id='" + news_id + "' AND " + "currency='"+currency+"'"+ " AND " + "percent='"+percent+"'"+ " AND " + "date='"+date+"'"
      cur=sql_query(sql)
      if cur.rowcount < 1:
          return False
      else:
...
def get_news_id(url,date):
    sql = "SELECT id FROM " + table_news + " WHERE url='" + url + "'" +" AND date='"+date+"'"
    cur=sql_query(sql)
    if cur.rowcount < 1:
        return False
    else:
...
def if_exist_history_buy(news_id,percent,history_buy,date):
    sql = "SELECT id FROM " + table_history_buy + " WHERE news_id='" + str(news_id) + "' AND percent='" + str(percent) + "' AND date='" + str(date) + "'"+ " AND history_buy='" + str(history_buy) + "'"
    cur = sql_query(sql)
    if cur.rowcount < 1:
        return False
    else:
...
def do_history_buy(news_id,currency,price,percent,history_buy, date):
   if if_exist_history_buy(news_id,percent,history_buy,date) == False:
    ...
    cur = insert_db(table_history_buy, data_history)
#------------------------------------------------------------------------------------------------------------
...
def parce_currency(currency_thread_name,url,currency,date_add_news):
    ...
    while delta_time<max_seconds:
        ...
        quantity=round(buy_limit / float(dates[currency]['btc']),3)
        ...
        if percent > percent_to_buy:
            if if_exist_currency(str(news_id), str(currency), str(percent), str(date_add_buy_currency))==False:
                ...
                buy_currency = insert_db(table_buy, data_buy)
                ...
                if status=='error':
                   ...
                   t_sell.start()
                   logging.debug(" Add currency with news_id = " + str(news_id))
                   return True
            else:
...
def get_currency_percent():
   ...
   html=get_html_by_proxy(url_currency)
   #if html==False:
...
def parse_chanel(thread_name,url,currency):
    while True:
      ...
      if flag == True:
        if exist_news==False:
          print ("Find news")
          logging.debug("Find news")
          res_twitter = insert_db(table_news, data)
...
#dates=get_currency_percent()
#print(dates)
#quantity = round(buy_limit / float(dates['LTC']['btc']), 5)
#print(quantity)
#html=get_html_by_proxy('http://www.mail.ru')
#print(html)
#btc=get_btc()
#print(btc)
#sys.exit(1)
