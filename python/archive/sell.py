...
def get_for_sell():
    sql = "SELECT * FROM " + table_buy + " WHERE was_sell='0'"
    cur = sql_query(sql)
    if cur.rowcount < 1:
        return False
    else:
...
def do_history_sells(buy_id, currency, price, history_sells, date_sell_currency):
     if if_exist_history_sells(buy_id,price,history_sells,date_sell_currency)==False:
        ...
        cur=insert_db(table_history_sells, data_history)
#------------------------------------------------------------------------------------------
def do_sell(news_id,buy_id,cause,currency,btc,price,quantity_btc,quantity_price,quantity_price_commission,amount_binance_buy,amount_dollar_binance_buy,date):
    ...
    profit= btc_usd*(price*quantity_price-btc*quantity_btc)
    profit_commision = btc_usd*(price*quantity_price_commission-btc*quantity_btc)
    ...
    insert_db(table_sells, data_sell)
    if test_script==True:
     data_update=[
        ['was_sell',1]
      ]
    ...
    update_db(table_buy,data_update,buy_id)
...
def if_exist_currency_sell(buy_id,currency):
      sql = "SELECT id FROM " + table_sells + " WHERE buy_id='" + str(buy_id) + "'"
      cur = sql_query(sql)
      if cur.rowcount < 1:
        return False
      else:
...
def if_exist_history_sells(buy_id,price,history_sells,date):
      sql = "SELECT id FROM " + table_history_sells + " WHERE buy_id='" + str(buy_id) + "' AND price='" + str(price)+ "' AND date='" + str(date)+"' AND history_sells='" + str(history_sells)+"'"
      cur = sql_query(sql)
      if cur.rowcount < 1:
        return False
      else:
...
def find_for_sell(sell_name,news_id,buy_id,currency,btc,amount_binance_buy,amount_dollar_binance_buy):
    ...
    cause = 'Не определена'
    history_sells = 0
    quantity_btc = round(buy_limit / float(btc), 5)
    while delta_time < max_seconds_sell:
        time.sleep(1)
        # если первый заход
        if current_price==0:
            current_price=btc
        ...
        try:
         ...
         price = float(response['price'])
            ...
            continue
        quantity_price =round(buy_limit / float(price),5)
        quantity_price_commission = round(buy_limit / float(price) - (commission * buy_limit / float(price) / 100), 5)
        print("Work " + sell_name+';'+ str(price)+';'+str(delta_time) )
        date_sell_currency = time.strftime('%Y-%m-%d %H:%M:%S')
        delta=(percent_to_sell*btc) / 100
        # если упало на 3%, то...
        if current_price-price > delta:
            #если до спада был рост, то продаем
          if currency_growth==True:
              cause = 'Валюта росла и упала на ' + str(percent_to_sell) + " % "
          else:
           cause = 'Валюта упала на '+str(percent_to_sell)+" % "
          history_sells=1
         ...
         do_history_sells(buy_id, currency, price, history_sells, date_sell_currency)
        delta_time = time.clock() - start_time
    cause = 'Валюта продана по истечению '+str(max_seconds_sell)+' сек.'
    history_sells = 1
    do_history_sells(buy_id, currency, price, history_sells, date_sell_currency)
    do_sell(news_id, buy_id, cause, currency, btc, price, quantity_btc, quantity_price, quantity_price_commission,amount_binance_buy, amount_dollar_binance_buy, date_sell_currency)
